...
def register(mymail):
    ...
    for i in m:
        if i == 'january':
            for d in m_31:
                m[i].objects.get_or_create(email=mymail, date="2020-1-" + str(d), afternoon=1, night=1)
        if i == 'february':
            for d in m_28:
                m[i].objects.get_or_create(email=mymail, date="2020-2-" + str(d), afternoon=1, night=1)
        if i == 'march':
            for d in m_31:
                m[i].objects.get_or_create(email=mymail, date="2020-3-" + str(d), afternoon=1, night=1)
        if i == 'april':
            for d in m_30:
                m[i].objects.get_or_create(email=mymail, date="2020-4-" + str(d), afternoon=1, night=1)
        if i == 'may':
            for d in m_31:
                m[i].objects.get_or_create(email=mymail, date="2020-5-" + str(d), afternoon=1, night=1)
        if i == 'june':
            for d in m_30:
                m[i].objects.get_or_create(email=mymail, date="2020-6-" + str(d), afternoon=1, night=1)
        if i == 'july':
            for d in m_31:
                m[i].objects.get_or_create(email=mymail, date="2020-7-" + str(d), afternoon=1, night=1)
        if i == 'august':
            for d in m_31:
                m[i].objects.get_or_create(email=mymail, date="2020-8-" + str(d), afternoon=1, night=1)
        if i == 'september':
            for d in m_30:
                m[i].objects.get_or_create(email=mymail, date="2020-9-" + str(d), afternoon=1, night=1)
        if i == 'october':
            for d in m_31:
                m[i].objects.get_or_create(email=mymail, date="2020-10-" + str(d), afternoon=1, night=1)
        if i == 'november':
            for d in m_30:
                m[i].objects.get_or_create(email=mymail, date="2020-11-" + str(d), afternoon=1, night=1)
        if i == 'december':
            for d in m_31:
                m[i].objects.get_or_create(email=mymail, date="2020-12-" + str(d), afternoon=1, night=1)
...
def save_attenadance(request):
    ...
    obj.afternoon = int(request.POST["noon"])
    obj.night = int(request.POST["night"])
    obj.save()
...
def get_records(request, month):
    ...
    for o in obj:
        ...
        attendance = [o.afternoon, o.night]
        if current_month == m_name:
            if record_day > current_day:
                continue
        d = {"date": str(date), "attendance": attendance}
        mydict.append(d)
